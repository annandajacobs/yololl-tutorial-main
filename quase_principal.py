...
while True:
    ...
    for box in detections:
        ...
        if cls in model.names:  # Verifica se a classe detectada está nas classes treinadas
            ...
            texto_placa = pytesseract.image_to_string(placa_thresh, config="--psm 11")  # Teste com psm 11
            ...
            if texto_extraido:
                texto_da_placa = texto_extraido.group()

                # Exibir o texto somente se a confiança for superior ou igual a 70%
                if conf >= 0.7:  # 70% de confiança
                    cv2.putText(frame, f"Texto: {texto_da_placa.strip()}", (x1, y2 + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
